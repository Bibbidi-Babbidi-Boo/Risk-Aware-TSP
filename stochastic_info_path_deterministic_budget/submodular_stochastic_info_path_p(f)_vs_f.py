...
def best_edge_gain(e, Hf, reward, edges, raster, length, tour_points, current_mean_reward, current_var_reward, current_mean_budget, M, beta, max_reward, max_edge_length, current_edges, current_lengths):
    ...
    H_marginal = HfUe - Hf
    return H_marginal, HfUe, mu, var, mu_budget, pt, f
...
def main():
    ...
    for ta in range(1):
        ...
        max_length = max(M.edge_length)
        ## Save file name
        file_name = '/home/rishab/Risk-Aware-TSP/plots/stochastic_info_path_deterministic_budget/stochasatic_information_p(y)_vs_f_alpha_vs_beta=0.0.csv'
                    ...
                    while edges!=[] and len(M.tour)<n:
                        ...
                        if ret == True:
                            ...
                            for e in range(len(edges)):
                                H_marginal, HfUe, mu, var, mu_budget, pt, f = best_edge_gain(e, Hf, reward, edges, raster, length, tour_points, current_mean_reward, current_var_reward, current_mean_budget, M,  beta, max_reward, max_length, current_edges, current_lengths)
                                Hm.append([H_marginal, HfUe, e, mu, var, mu_budget, pt, f])
                            Hm.sort(reverse=1)
