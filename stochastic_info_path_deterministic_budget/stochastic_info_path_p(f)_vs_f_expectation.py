...
def best_edge_gain(e, Hf, reward, fail, length, tau, alpha, current_mean, current_var, current_mean_budget, beta, m, n, subtour, tour_temp, max_edge_reward, edges, sum_of_largest_edges):
    tour = list(tour_temp)
    tour.append(edges[e])
    mu = reward[e]
    var =  fail[e]
    ...
    for i in range(500):
        t = np.random.normal(mu+avg, sqrt(var+var_avg)) +  max_edge_reward*len(tour)
                    ...
                    while edges!=[] and len(M.tour)<n:
                        ...
                        if ret == True:
                            ...
                            for e in range(len(edges)):
                                H_marginal, HfUe = best_edge_gain(e, Hf, reward, fail, length, M.tau, M.alpha, current_mean, current_var, current_mean_budget, beta, max_edge_length, n, subtour, M.tour, max_edge_reward, edges, sum_of_largest_edges)
